...
// Decreases stock for each ordered item that has stock tracking enabled
// (stock !== null), and automatically marks a product sold out once it
// hits zero. Products without stock tracking (stock === null) are left
// alone — their availability stays on the manual Sold Out toggle.
// Uses an atomic $inc so concurrent orders can't push stock negative.
async function decrementStockForOrder(items) {
  for (const item of items) {
    ...
    try {
      ...
      // Don't let a stock-update hiccup fail the order itself — the order
      // has already been placed successfully at this point.
      console.error("Could not update stock for product", item.productId, err.message);
    }
  }
}
